import { addressBodySchema, ErrorCode } from '@commerce/contracts';
import { addAddressAction } from '@/server/address-actions';
...
/**
 * 접착제. 로직은 전부 `addAddressAction`에 있고 그쪽은 테스트가 있다.
...
export async function POST(request: Request): Promise<Response> {
  const body = addressBodySchema.safeParse(await request.json());
  if (!body.success) {
  ...
  return handleBff(async () =>
    toResponse(
      await addAddressAction(body.data, {
        baseUrl: apiBaseUrl(),
