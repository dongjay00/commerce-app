import { ErrorCode, placeOrderBodySchema } from '@commerce/contracts';
import { apiBaseUrl } from '@/server/api-client';
import { handleBff, toResponse } from '@/server/bff-response';
import { placeOrderAction } from '@/server/order-actions';
import { cookieTokenStore } from '@/server/session';

/**
 * 접착제. 로직은 전부 `placeOrderAction`에 있고 그쪽은 테스트가 있다.
...
export async function POST(request: Request): Promise<Response> {
  const body = placeOrderBodySchema.safeParse(await request.json());
  if (!body.success) {
  ...
  return handleBff(async () =>
    toResponse(
      await placeOrderAction(body.data, {
        baseUrl: apiBaseUrl(),
