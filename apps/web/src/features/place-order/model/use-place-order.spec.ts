...
import { type PlaceOrderOutcome, usePlaceOrder } from './use-place-order';
...
describe('usePlaceOrder', () => {
  it('성공하면 orderId와 status를 돌려준다', async () => {
    ...
    await act(async () => {
      outcome = await result.current.placeOrder(ADDRESS_ID);
    });

    expect(outcome).toEqual({ ok: true, result: { orderId: ORDER_ID, status: 'PAID' } });
  });
  ...
  it('결제 거절도 성공이고 status가 PAYMENT_FAILED다', async () => {
    ...
    expect(outcome).toEqual({
      ok: true,
      result: { orderId: ORDER_ID, status: 'PAYMENT_FAILED' },
    });
    // 거절은 오류가 아니다 — 빨간 문구를 띄우지 않는다.
    expect(result.current.error).toBeNull();
  });

  it('재고가 부족하면 실패 문구를 돌려주고 담는다', async () => {
    server.use(
      http.post('/api/orders', () =>
    ...
    // 돌려주는 문구와 `error`가 같다 — 새로고침으로 버튼이 사라진 뒤에도
    // 부모가 같은 이유를 그릴 수 있어야 한다(최종 리뷰 I2).
    expect(outcome).toEqual({ ok: false, message: MESSAGES[ErrorCode.INSUFFICIENT_STOCK] });
  ...
  it('응답이 계약 형태가 아니면 실패다', async () => {
    ...
    expect(outcome).toEqual({ ok: false, message: MESSAGES[ErrorCode.INTERNAL_ERROR] });
