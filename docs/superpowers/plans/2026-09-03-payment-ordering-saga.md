...
`pnpm verify` exit 0 · 150 파일 · 1,280 통과 / 6 스킵 · 의존성 위반 0 (483 모듈, 2,199 의존성).

### 기능 (스펙 §13 중 이 계획의 몫)

| 기준 | 결과 | 증거 |
|---|---|---|
| 장바구니 → 주문 → 결제 성공 → 예약 확정 | ✅ | `test/saga/place-order.integration.spec.ts` |
| 결제 거절 시 예약 해제 + `PAYMENT_FAILED` | ✅ | `payment-declined.integration.spec.ts` |
| `PAID` 취소 시 환불 + 재고 복원 | ✅ | `cancel-paid-order.integration.spec.ts` |
| TTL 만료 시 재고 회복 + 주문 실패 | ✅ | `reservation-expiry.integration.spec.ts` |
| 가격·주소가 스냅샷으로 남고 원본이 바뀌어도 그대로 | ✅ | `place-order`의 `'상품 가격이 바뀌어도…'` |
...
| 도메인이 다른 컨텍스트를 공개 API로도 부르지 않음 | ✅ grep 0건 |
| `InProcessInventoryAdapter` 한 파일만 고쳐 호출 경로 교체 가능 | ✅ — 그 파일의 생성자 인자를 HTTP 클라이언트로 바꾸면 `InventoryReserver` 인터페이스는 그대로이므로 `PlaceOrderService`·모듈 배선·테스트가 전부 무변경이다 |

### 테스트

| 기준 | 결과 |
|---|---|
| 같은 계약 테스트가 in-memory와 Prisma 양쪽에서 통과 | ✅ `Cart`·`Order`·`Payment` 각각 2구현 |
| 같은 이벤트가 두 번 배달돼도 부수 효과가 한 번만 | ✅ 재고(`place-order`)·환불(`cancel-paid-order`) 양쪽에서 확인 |
| `modules/*/domain/**` 95/90 | ✅ **99.7% / 99.8%** |
| `modules/*/application/**` 90/85 | ✅ **99.3% / 98.5%** |
| Outbox 원자성(계획 1) 유지 | ✅ `shared/infrastructure/outbox/outbox-event.publisher.integration.spec.ts` |

---

## 부록 — 실행 중 계획이 틀렸던 곳

프루브가 매번 같은 종류의 결함을 찾았다: **"테스트가 통과한다"와 "테스트가 그 성질을
지킨다"는 다르다.** 아래 중 앞의 셋은 테스트를 강화해 고쳤고, 뒤의 둘은 프로덕션
코드의 실제 결함이었다.

**테스트가 아무것도 검증하지 않던 곳 (강화함)**

1. **태스크 2 — 주문 목록 인덱스.** 정렬 방향을 인덱스에서 빼도 인덱스 이름은 플랜에
   그대로 남고 `Sort` 노드만 추가된다(실측). 원래 단언은 회귀를 전혀 못 잡았다 →
   `expect(planText).not.toContain('Sort')` 추가.
2. **태스크 5 — 결제 시도 순서.** 행이 둘뿐이면 물리적 순서가 정렬 순서와 같아
   `orderBy`를 지워도 통과했다 → 나중 시각을 먼저 삽입하도록 계약을 고치고, in-memory
   구현에도 같은 정렬을 넣었다.
3. **태스크 12 — 모르는 SKU.** 바깥 `missing` 검사를 지워도 안쪽 방어선이 같은 예외를
   던져 통과했다. 바깥 검사의 진짜 값은 **빠진 SKU를 전부 나열하는 것**이다 →
   여러 SKU가 빠진 케이스 추가.

**프로덕션 코드의 실제 결함 (사가 E2E가 찾음)**

4. **재고 부족으로 예약이 실패하면 주문이 `PENDING_PAYMENT`로 영원히 남았다.**
   `assemble` 트랜잭션에서 주문이 이미 만들어지고 장바구니가 지워진 뒤이고, TTL
   스캔은 예약을 훑는데 예약이 없어 아무도 그 주문을 끝내지 않는다. 고객의 주문
   목록에 "결제 대기"가 영구히 걸린다. → `PlaceOrderService.failOrder`를 추가해
   `PAYMENT_FAILED`로 끝낸다(PG 타임아웃과 다르다 — 거기서는 결제 여부를 알 수 없어
   `PENDING_PAYMENT`가 정답이다). 단위 테스트가 이것을 못 잡았으므로 케이스도 추가했다.
5. **`@OnEvent`의 `suppressErrors` 기본값이 `true`라 모든 구독자 예외가 삼켜지고 있었다.**
   Nest가 리스너 예외를 삼키면 `emitAsync`가 정상 완료로 보이고, 릴레이는 전송 성공으로
   판단해 `published_at`을 채운다 — 실패한 이벤트가 영영 사라지고 계획 1이 만든
   재시도·백오프·데드레터가 전부 죽은 코드가 된다. 일곱 개 `@OnEvent`에
   `{ suppressErrors: false }`를 넣고, E2E에 `failedOutboxCount()` 단언을 더해 그
   침묵이 다시 생기면 깨지게 했다.

**프루브가 판별하지 못한 것 (기록만)**

- **태스크 4 (a)** — 환불에서 "상태 먼저, PG 나중" 순서. 순차 호출만 하는 단위
  테스트는 순서를 구분하지 못한다. 지키는 장치는 코드 리뷰뿐이다.
- **태스크 10 (a)** — 값 객체 생성을 트랜잭션 안으로 옮겨도 통과한다. 중요한 순서는
  "트랜잭션 밖"이 아니라 **"`save` 앞"**이고, 계획의 추론이 틀렸다.
- **태스크 13 (a)** — 취소의 `if (changed)` 가드를 지워도 통과한다. `pullEvents()`가
  두 번째엔 빈 배열이라 `publish([])`가 되기 때문이고, 진짜 보증은 `Order.cancelBy`의
  `false` 반환에 있다.
- **태스크 13 (c) / 태스크 16 (a)** — 인가가 어느 계층에 있는지, 도메인 코드 문자열이
  무엇인지는 단위 테스트로 구분되지 않는다. 전자는 스펙 §5.5가 HTTP 아닌 경로를
  이유로 도메인을 고른 것이고, 후자는 출처가 하나여서 양쪽이 함께 움직이는 것이 정답이다
  (와이어에 나가는 것은 `ErrorCode`이고 계약이 고정한다).
- **태스크 17 (b)** — 트랜잭션 안 재조회를 지워도 순차 호출 테스트는 통과한다.
  동시 요청에서만 드러나고, 예약 전이의 동시성 스위트는 없다(아래 이월).
- **태스크 18 (b) / 태스크 19 (a)** — 이벤트 이름 오타와 `inject` 배열 순서 뒤바뀜은
  타입 검사·DI 해석·통합 테스트를 **전부 통과한다.** 태스크 20의 사가 E2E가 둘을 잡는
  유일한 장치임을 실측으로 확인했다.
...
- **PG 웹훅이 주문을 움직이지 않는다** (편차 3) — 비동기 승인 경로는 백로그.
- **PG 웹훅에 서명 검증이 없다** — 이 엔드포인트는 공개돼 있다. 통합 테스트가
  "토큰 없이 불러도 200"을 고정하고 있으므로, 서명 검증을 넣으면 그 테스트가 깨진다.
- **부분 환불이 없다** (편차 4).
- **역할 기반 인가가 없다** (편차 6) — catalog·inventory의 쓰기 엔드포인트가 인증만 걸려 있다.
- **`OutboxRelay`에 `FOR UPDATE SKIP LOCKED`가 없다** (편차 5) — 소비자 멱등성으로
  갚았고 태스크 20·21이 그것을 검증했다. 멀티 인스턴스로 가는 시점에 넣는다.

**실행 중 새로 생긴 것**

- **`ProductQuery.findSkus`의 N+1을 잡는 장치가 없다** (태스크 16 프루브 b) —
  SKU마다 `findFirst`를 부르도록 바꿔도 모든 테스트가 통과한다. 쿼리 수를 세는
  장치가 필요하다.
- **예약 전이의 동시성 스위트가 없다** (태스크 17 프루브 b) — 계획 3이 재고에 대해
  만든 것과 같은 것이 예약 상태 전이에는 없다.
- **`inventory.module.ts:2`에 불필요한 biome 억제 주석이 남아 있다** — 계획 3의
  태스크 15가 남긴 경고(`suppressions/unused`)이고 verify를 막지는 않는다.
- **`PlaceOrderService`의 `inject` 배열을 지키는 장치가 사가 E2E뿐이다** — 인자가
  열 개이고 넷이 ACL이라 위험이 크다. 배선을 단언하는 더 싼 테스트가 있으면 좋다.

**계획 5의 범위** — 프론트엔드 상점 UI(FSD `entities`/`features`/`widgets`/`views`),
MSW 핸들러의 계약 검증(스펙 §9.9), Playwright 브라우저 E2E 6~8 시나리오(스펙 §9.10),
`docs/architecture.svg` 생성(graphviz가 있는 환경).
